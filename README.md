...
일본의 [経県値](https://uub.jp/kkn/)(경현치) 개념을 바탕으로, 한일 양국 동시 기록·시정촌 정밀도·GPS 도장으로 재해석했습니다.
...
- **도장 기록** — 지역을 탭할 때마다 등급 순환: 미답(0) → 통과(1) → 접지(2) → 방문(3) → 숙박(4) → 거주(5)
- **양국 동시 뷰** — 일본·한국 지도를 한 화면에 띄우고 합산 레벨 + 국가별 통계로 기록
- **시정촌/시군구 단위** — 관리 화면(목록·미니맵)에서 기초 지역별 도장, '기초 지역' 토글로 전국을 시정촌 단위로 열람
