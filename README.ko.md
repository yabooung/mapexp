...
- **양국 동시 뷰** — 일본·한국 지도를 한 화면에 띄우고 국가별 여행자 레벨·통계로 기록 (여행자 레벨은 보고 있는 나라를 따라 표시)
...
- **GPS 현재 위치 도장** — 버튼 한 번으로 지금 있는 지역(광역+기초)을 자동 판정해 도장. 어느 나라에 있든 자동 인식
  - GPS 인증 기록은 생성 후 시간 수정·삭제 불가 (수동 기록은 과거 날짜 자유 입력)
- **게임화** — 여행자 레벨(국가별), 한자 낙관 도장첩 — 마일스톤 도장과 **지역 완주 도장**(한 광역의 모든 기초 방문, 또는 시코쿠·전라 등 권역 완주 시 발급, 藍 파랑), 지역별 점수·기초 점수 통계
- **회원 · 클라우드 동기화** *(선택)* — 이메일 또는 구글로 로그인. 로그인하면 기록이 서버에 자동 저장되고 기기 간에 동기화되며, 기존 로컬 기록과 손실 없이 병합됩니다. 로그인하지 않으면 모두 로컬에만 저장 — 계정은 opt-in
- **월별 일지** *(회원)* — 방문 기록이 월별로 자동 정리되고, 한 달에 여러 개의 일지를 직접 쓰고 수정·삭제할 수 있습니다. 일지의 월·날짜를 자유롭게 바꾸고, 월별·연별로 히스토리를 볼 수 있습니다
- **공유** — SNS 이미지 카드(Lv·EXP 게임식 점수, 순위 티어, 지명 라벨·도장첩 표시 옵션, 일본/한국/양국 × 광역/기초/둘다), 지역별 카드 프리뷰 공유, QR 코드, 읽기 전용 공유 링크, JSON 파일 교환, 계정 없는 기기 간 코드 동기화. 받은 지도는 내 기록과 나만/상대만/둘 다 3색으로 '겹쳐보기' 비교 가능
- **다국어 UI·라우트** — 한국어 · English · 日本語, 언어별 페이지(`/ko`·`/ja`·`/en`)
- **PWA** — 홈 화면 설치, 지도 데이터 오프라인 캐싱
- **로컬 우선 · 프라이버시** — 로그인하지 않으면 모든 기록과 위치 정보는 브라우저에만 저장됩니다. GPS 좌표는 기기에서만 쓰이고 서버로 전송하지 않습니다. [개인정보처리방침](https://mapexp.app/privacy)·[이용약관](https://mapexp.app/terms) 참고

## 기술 스택

Next.js 16 · React 19 · TypeScript · Tailwind CSS 4 · Leaflet(react-leaflet) · Zustand · Turf.js · d3-geo · Supabase(인증·클라우드 동기화)
