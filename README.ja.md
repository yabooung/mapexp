...
**▶ https://mapexp.app — インストール不要ですぐ使えます**
...
- **アカウント・クラウド同期** *(任意)* — メールまたはGoogleでログイン。ログインすると記録がクラウドに自動保存され、端末間で同期され、既存のローカル記録と欠損なく統合されます。ログインしなければすべてローカルのみ — アカウントはオプトイン
- **月別ジャーナル** *(会員)* — 訪問記録が月別に自動整理され、1か月に複数の記録を自分で書いて編集・削除できます。記録の月・日付を自由に変更でき、月別・年別に履歴を見られます
- **共有** — SNS画像カード（Lv・EXPのゲーム式スコア、ランク称号、地名ラベル・スタンプ帳の表示オプション、日本／韓国／両国 × 広域／市区町村）、地域別カードのプレビュー共有、QRコード、閲覧専用リンク、JSONファイル交換、アカウント不要の端末間コード同期。受け取った地図は自分の記録と「自分のみ／相手のみ／両方」の3色で重ねて比較できます
- **多言語UI・ルート** — 한국어 · English · 日本語、言語別ページ（`/ko`・`/ja`・`/en`）
- **PWA** — ホーム画面への追加、地図データのオフラインキャッシュ
- **ローカル優先・プライバシー** — ログインしなければ記録と位置情報はすべて端末内にのみ保存されます。GPS座標は端末上でのみ使用し、サーバーへ送信しません。[プライバシーポリシー](https://mapexp.app/privacy)・[利用規約](https://mapexp.app/terms)をご覧ください

## 技術スタック

Next.js 16 · React 19 · TypeScript · Tailwind CSS 4 · Leaflet (react-leaflet) · Zustand · Turf.js · d3-geo · Supabase（認証・クラウド同期）
