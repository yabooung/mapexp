...
interface RegionExp {
  ...
  level: ExpLevel;            // 경치 레벨 (0-5)
  visitedAt?: Date;           // 최초 방문일
  lastVisitedAt?: Date;       // 최근 방문일
  visitCount?: number;        // 방문 횟수 (레벨 5 판정용)
  totalNights?: number;       // 누적 숙박일 (레벨 5 판정용)
...
|------|------|--------|------|------|------|
| 0 | 미방문 | ⚪ | 아직 가보지 않음 | - | 0점 |
| 1 | 통과 | 🚶 | 지나가기만 함 | 공항 환승, 고속도로, 기차역 | 1점 |
| 2 | 짧은 방문 | 📍 | 잠깐 들름 | 2-3시간 이하 (관광지 1-2곳) | 2점 |
| 3 | 긴 방문 | 🗺️ | 제대로 돌아봄 | 반나절-하루 (여러 관광지) | 3점 |
| 4 | 숙박 | 🏨 | 1박 이상 | 1박 이상 숙박 | 4점 |
| 5 | 마스터/거주 | ⭐ | 완전 정복 | 3회+ 방문 & 3박+ OR 30일+ 거주 | **8점** |

**레벨 5 달성 조건 (둘 중 하나):**
- **조건 A (마스터)**: 3회 이상 방문 + 누적 3박 이상 숙박
- **조건 B (거주)**: 30일 이상 연속 체류

**레벨 5 가중치:**
- 희소성과 달성 난이도를 반영하여 **8점 (2배 가중치)** 부여
- 깊이 있는 탐방을 장려하고 재방문 동기 부여
...
**레벨 5 선택 시 확인 메시지 (Phase 1):**
```
"⭐ 마스터/거주는 다음 조건 중 하나에 해당합니다:
 • 3회 이상 방문 + 누적 3박 이상
 • 30일 이상 거주
