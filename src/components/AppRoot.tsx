...
import { useT, useLang, muniTerm, type Lang } from '@/lib/i18n'
...
function HomeContent({ initialLang }: { initialLang?: Lang }) {
  ...
  // 언어 초기화:
  // - 언어별 라우트(/ja 등)로 들어오면 그 언어를 명시적 선택으로 보고 UI 언어를 맞춘다
  // - 아니면 첫 방문에 한해 브라우저 언어로 UI 언어와 기본 국가를 정한다 (한국어 → 한국 지도)
  // 주의: 다른 효과들이 스토어를 건드려 mapexp_data가 생기기 전에 판정해야 하므로 가장 먼저 실행
  useEffect(() => {
    try {
      const store = useMapExpStore.getState()
      const firstVisit = !localStorage.getItem('mapexp_data')
      if (initialLang) {
        store.updateSettings({ language: initialLang, mapLanguage: 'auto' })
        if (firstVisit) store.setCountry(initialLang === 'ko' ? 'korea' : 'japan')
        return
      }
...
/**
 * 앱 본체. 언어별 라우트(/ko·/ja·/en)와 루트(/)가 공유하는 클라이언트 루트.
 * initialLang이 주어지면 해당 언어로 UI를 초기화한다 (SEO용 언어별 라우팅).
 */
